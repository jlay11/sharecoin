...
QString TransactionDesc::toHTML(CWallet *wallet, CWalletTx &wtx)
{
...

    {
        ...
        if (wtx.IsCoinBase() && nCredit == 0)
        {
            ...
            if (wtx.IsInMainChain())
                strHTML += tr("(%1 matures in %2 more blocks)")
                        .arg(FreicoinUnits::formatWithUnit(FreicoinUnits::BTC, nUnmatured))
        ...
        else if (nNet > 0)
        {
            ...
            // Credit
            //
            strHTML += tr("<b>Credit:</b> ") + FreicoinUnits::formatWithUnit(FreicoinUnits::BTC, nNet) + "<br>";
        }
        else
        {
            ...
            if (fAllFromMe)
            {
                ...
                BOOST_FOREACH(const CTxOut& txout, wtx.vout)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + FreicoinUnits::formatWithUnit(FreicoinUnits::BTC, -txout.GetPresentValue(wtx.GetDepthInMainChain())) + "<br>";
                }
                ...
                if (fAllToMe)
                {
                    ...
                    strHTML += tr("<b>Debit:</b> ") + FreicoinUnits::formatWithUnit(FreicoinUnits::BTC, -nValue) + "<br>";
                    strHTML += tr("<b>Credit:</b> ") + FreicoinUnits::formatWithUnit(FreicoinUnits::BTC, nValue) + "<br>";
                }

                int64 nTxFee = nDebit - wtx.GetValueOut(wtx.GetDepthInMainChain());
                if (nTxFee > 0)
                    strHTML += tr("<b>Transaction fee:</b> ") + FreicoinUnits::formatWithUnit(FreicoinUnits::BTC,-nTxFee) + "<br>";
            }
            else
            {
                //
        ...
        strHTML += tr("<b>Net amount:</b> ") + FreicoinUnits::formatWithUnit(FreicoinUnits::BTC,nNet, true) + "<br>";
                ...
                BOOST_FOREACH(const CTxIn& txin, wtx.vin)
                {
                    ...
                    if(txdb.ReadDiskTx(prevout.hash, prev))
                    {
                        if (prevout.n < prev.vout.size())
                        {
                            ...
                            strHTML = strHTML + " Amount=" + FreicoinUnits::formatWithUnit(FreicoinUnits::BTC,vout.GetPresentValue(wtx.GetDepthInMainChain()));
