...
std::string HelpMessage()
{
    ...
    strUsage += string() +
        _("\nSSL options: (see the Freicoin Wiki for SSL setup instructions)") + "\n" +
...
bool AppInit2()
{
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  Freicoin is probably already running."), GetDataDir().string().c_str()));
